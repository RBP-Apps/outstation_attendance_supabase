...
const TravelHistory = () => {
  ...
  const fetchTravelHistory = async () => {
    ...
    try {
      // Fetch from Supabase travelling table
      ...
      const formattedHistory = rows.map((row, index) => {
        ...
        const noOfDays = row.stay_date; // stay_date stores noOfDays in the current flow
        const accommodationType = row.accommodation_type;
        const travelDate = row.travel_date;
        const stayBillLink = row.stay_bill;
        const foodingDates = row.fooding_date;
        const foodingBillLinks = row.fooding_bill;
        const travelReceiptLink = row.travel_receipt;
        const localTravelType = row.local_travel_type;
        const localTravelReceiptLink = row.local_travel_receipt;
        const returnDate = row.return_date;
        const returnTicketLink = row.return_ticket;
        const advanceAmount = row.advance_amount;
